...
char hflag; /* hardcoded parser: LR or LL */
char pflag; /* predictive parser: LL(1) */
char dflag;
char lflag;
char rflag;
char tflag;
char vflag;
char xflag; /* embeded lexical analyzer */
/* The version number is also stored in skeleton.c.  */
char *yaccversion = "Berkeley Yacc (+28-cygwin-990830)";
char emitident;
...
usage()
{
    fprintf(stderr, "usage: %s [-dlrtvVhp] [-Q[y|n]] [-b file_prefix] [-o outputfile] filename\n", myname);
    /* prs */
...
create_file_names()
{
	...
	strcpy(output_file_name + len, hflag ? CODE_SUFFIX : OUTPUT_SUFFIX);
      }
