...
static int last_nitem;

static void
default_action() {
    if (hflag && !pflag) {
	register short len = nitems - last_nitem -1;
        fprintf(action_file, "reduce_%d:\n", nrules - 2);
	if (len > 0)
	    fprintf(action_file, "\tyyval = (tos - %d)->val;\n\ttos -= %d;\n",
	    			 len, len);
	fprintf(action_file, "\ttos->val = yyval;\n\tgoto nonterminal_%s;\n",
			     plhs[nrules]->name);
    	last_nitem = nitems;
    }
}
...
end_rule()
{
    ...
    if (!last_was_action) default_action();
...
copy_action()
{
    ...
    char *stk = "yyvsp[%d]", *stktag = "yyvsp[%d].%s";

    if (hflag && !pflag) {
        stk = "(tos-1)[%d].val";
        stktag = "(tos-1)[%d].val.%s";
    }
